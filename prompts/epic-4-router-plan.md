...
> **Status:** Tasks 0–4 COMPLETE. Task 4 (guards & reverts) is GREEN: 17/17 unit + fuzz tests pass; router now at **100% line/statement/branch/function coverage** (Task 3's revert-branch gap closed). Added `test/mocks/MockReentrantOrg.sol` (re-enters `router.donate()` inside its `donate()` callback) — `nonReentrant` reverts the outer tx with `ReentrancyGuardReentrantCall`. New tests: zero-org, zero-amount, no-allowance, insufficient-balance, reentrancy. Note: `expectRevert(bytes4)` only matches bare 4-byte reverts in forge v1.7.1, so parameterized OZ errors (`ERC20InsufficientAllowance`/`ERC20InsufficientBalance`) are asserted via `abi.encodeWithSelector(...)`. No production-contract change was needed — the `nonReentrant` guard landed in Task 3. Tasks 1–3 remain GREEN. Resume at **Task 5** — forked Base mainnet integration (Epic-5 gated) via `/tdd`.
