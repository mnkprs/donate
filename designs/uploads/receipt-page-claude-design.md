...
The receipt above is a **vanilla Endaoment donation** rendered by Philotimo. Philotimo does not yet route donations through its own smart contract, so there is **no Philotimo platform fee in this transaction**. The tracker still shows a 5th stage for the platform fee, but it must render as **future / not-yet-active** with honest copy (see Pizza Tracker section). Do not show a fake Philotimo fee that doesn't exist on-chain — the entire product value is verifiability.
...
- **Philotimo fee:** not yet active — shown as future stage
...
2. **The Pizza Tracker (the differentiator — make this beautiful).** A horizontal 5-stage tracker on desktop, vertical on mobile. Each *active* stage is a stop on the path the money took, with timestamp and a "verify ↗" link to BaseScan. Stage 4 is **special**: it represents Philotimo's future role and must render as visibly inactive (greyed dot, lighter text, no checkmark, no verify link) — never as a fake complete step.
   ...
   - **4. Philotimo fee** ⏳ **inactive / future** — copy reads: *"Future Philotimo donations will route through our 1% platform fee here. This receipt is for an existing Endaoment donation, so no Philotimo fee was charged."* Render as dimmed/dashed, distinctly visually different from active stages. No verify link.
   - **5. Settled** ✓ active — USDC arrived at charity's on-chain address (`0x10e9…eb82`) with 15M+ confirmations

   Stages 1, 2, 3, 5 are complete; this is a *replay*, not a live tracker. Use a calm checkmark or filled dot, not aggressive green. Each active stage has a subtle hover state revealing the relevant log + amount. Use the brand accent (Indigo `#533afd` per DESIGN.md) for the active accent color, not generic success-green. The visual contrast between the 4 active stages and the 1 inactive future stage is the **honesty signal** — donors should *feel* that Philotimo doesn't pretend to be more than it is yet.
